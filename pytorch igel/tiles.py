...
def generate_tiles_inference(image_file, out_dir, tile_size):
    img = cv2.imread(image_file, cv2.IMREAD_GRAYSCALE)
    h, w = img.shape
    for i in range(math.ceil(h / tile_size[1])):
        for j in range(math.ceil(w / tile_size[0])):
            rect = [j * tile_size[0], i * tile_size[1], tile_size[0], tile_size[1]] # x, y, w, h
            img_tile = utils.crop(img, rect[0], rect[1], rect[2], rect[3])
            file_name = utils.get_file_name(image_file) + ('_%i_%i_%i_%i' % (rect[0], rect[1], rect[2], rect[3]))
            cv2.imwrite(os.path.join(out_dir, file_name + '.jpg'), img_tile)



def generate_tile_cache_inference(image_dir, output_dir, **kwargs):
    ...
    with multiprocessing.Pool() as pool:
        ...
        for i in range(steps):
            ...
            low = i * process_batch
            high = min(low + process_batch, len(files))
            items = [(os.path.join(image_dir, img_file), output_dir, (kwargs['size'], kwargs['size'])) for img_file in files[low:high]]
            pool.starmap(generate_tiles_inference, items)