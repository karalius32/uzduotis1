...
class Model(nn.Module):
    def __init__(self, in_channels, num_of_classes, filters=8, depth=3, activation="relu", activation_args=None):
        ...
        for i in range(depth):
            self.encoder_res_blocks.append(EncodingResBlock(in_channels=filters * (2 ** i), out_channels=filters * (2 ** (i + 1)), kernel_size=3, max_pool=True, max_pool_size=2, batch_norm=True, activation=activation, activation_args=activation_args))
        ...
        for i in range(depth):
            dec_in_channels = filters * (2 ** (depth - i)) * 4 if i > 0 else filters * (2 ** (depth + 1))
            self.decoder_res_blocks.append(DecodingResBlock(in_channels=dec_in_channels, out_channels=filters * (2 ** (depth - i)), kernel_size=3, up_sample_size=2, batch_norm=True, activation=activation, activation_args=activation_args))
        self.decoder1 = DecodingResBlock(in_channels=filters * 4, out_channels=filters, kernel_size=3, up_sample_size=2, batch_norm=True, activation=activation, activation_args=activation_args)
        # Outputs
        final_activation = "softmax" if num_of_classes > 1 else "sigmoid"
        conv_block = ConvBlock(in_channels=filters * 2, out_channels=num_of_classes * 2, kernel_size=3, batch_norm=True, activation=activation, activation_args=activation_args)
        final_conv = nn.Conv2d(in_channels=num_of_classes * 2, out_channels=num_of_classes, kernel_size=1)
        nn.init.xavier_normal_(final_conv.weight)
    ...
    def forward(self, input):
        ...
        dec = bottleneck
        for i in range(self.depth):
            dec = self.decoder_res_blocks[i](dec, skip_connections[self.depth - i])
        dec = self.decoder1(dec, skip_connections[0])
        # Output
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, batch_norm=True, activation=None, activation_args=None):
        super().__init__()
        self.zero_pad = nn.ZeroPad2d(padding=kernel_size // 2)
...
class AtrousConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, activation, activation_args):
        ...
        nn.init.kaiming_normal_(self.atrous_conv.weight)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, batch_norm=True, activation=None, activation_args=None):
        ...
        nn.init.kaiming_normal_(self.shortcut_conv.weight)
...
class DecodingResBlock(nn.Module):
    def __init__(self, in_channels, out_channels=8, kernel_size=3, up_sample_size=2, batch_norm=True, activation="relu", activation_args=None):
        ...
        self.upsample_conv = nn.Sequential(
            nn.UpsamplingBilinear2d(scale_factor=up_sample_size),
            ConvBlock(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, batch_norm=batch_norm, activation=activation, activation_args=activation_args)
        )
...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels=8, kernel_size=3, depth=3, activation="relu", activation_args=None):
        super().__init__()
        self.atrous_conv_blocks = nn.ModuleList([])
        for i in range(depth):
            dilation = 2 ** i
    ...
    def forward(self, input):
        ...
        pool = nn.functional.interpolate(input=pool, size=atrous_conv_outputs[0].shape[-2:], mode="bilinear")
        flow = torch.concatenate([*atrous_conv_outputs, pool], axis=1)
