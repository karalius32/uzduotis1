...
float helper_new::generate_scale(cv::Mat& image, const std::vector<int>& target_size) {
...
float helper_new::letterbox(cv::Mat& input_image, cv::Mat& output_image, const std::vector<int>& target_size) {
...
torch::Tensor helper_new::xyxy2xywh(const torch::Tensor& x) {
...
torch::Tensor helper_new::xywh2xyxy(const torch::Tensor& x) {
    auto y = torch::empty_like(x);
...
torch::Tensor helper_new::nms(const torch::Tensor& bboxes, const torch::Tensor& scores, float iou_threshold) {
    if (bboxes.numel() == 0)
...
torch::Tensor helper_new::non_max_suppression(torch::Tensor& prediction, float conf_thres, float iou_thres, int max_det) {
...
torch::Tensor helper_new::clip_boxes(torch::Tensor& boxes, const std::vector<int>& shape) {
...
torch::Tensor helper_new::scale_boxes(const std::vector<int>& img1_shape, torch::Tensor& boxes, const std::vector<int>& img0_shape) {
